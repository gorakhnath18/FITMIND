...
const DailyTasks = () => {
    ...
    // The helper function to determine button classes
    const getButtonClasses = (isCompleted, isLoading) => {
        ...
        if (isCompleted) {
            // Changed from green to purple for the completed state
            return `${baseClasses} bg-purple-500 text-white`;
        }
        // --- END OF CHANGE ---

        if (isLoading) {
            // Give the loading button the same color as the active button for that theme
            return `${baseClasses} ${theme === 'dark' ? 'bg-green-500' : 'bg-blue-500'}`;
        }
        if (theme === 'dark') {
            return `${baseClasses} bg-green-500 text-white hover:bg-green-600`;
        }
        // Default case: Light mode, not completed, not loading
